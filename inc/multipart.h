...
namespace multipart {
    ...
    ///
    struct part {
        std::vector<header> headers; ///< (optional) The first part before the data, Contains information regarding the type
                                     ///< of data and encoding
      ...
      private:
        ...
        void parse_section_head(std::string &lines, part &part) {
            while (!lines.empty()) {
                ...
                if (!line.empty()) {
                    size_t found_in = line.find("; ");
                    std::string header = line.substr(0, found_in);
                    if (found_in != std::string::npos)
                        line.erase(0, found_in + 2);
                    else
                        line = std::string();
                    ...
                    to_add.value =
                        std::pair<std::string, std::string>(header.substr(0, header_split), header.substr(header_split + 2));
                }

                // add the parameters
                while (!line.empty()) {
                    size_t found_in = line.find("; ");
                    std::string param = line.substr(0, found_in);
                    if (found_in != std::string::npos)
                        line.erase(0, found_in + 2);
                    else
                        line = std::string();
